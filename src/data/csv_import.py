...
POPUlATION_CSV_PATH = os.path.join(
    os.path.dirname(__file__), "../../data/raw/Electric_Vehicle_Population_Data.csv"
)
REGISTRATION_CSV_PATH = os.path.join(
    os.path.dirname(__file__),
    "../../data/raw/Electric_Vehicle_Title_and_Registration_Activity.csv",
)
...
INSERT OR IGNORE INTO population ({", ".join(pop_table_cols)})
    VALUES ({", ".join(["?"] * len(pop_table_cols))})
"""
reg_table_sql_insert = f"""
INSERT OR IGNORE INTO registration ({", ".join(reg_table_cols)})
...
def main() -> None:
    try:
        ...
        with sqlite3.connect(DB_PATH) as connection:
            ...
            print("csv rows copied into memory")
            ...
            try:
                ...
                connection.commit()
                cursor.execute("SELECT COUNT(*) AS row_count FROM population;")
                pop_total_rows = cursor.fetchall()[0][0]
                logger.info(
                    "Inserted %s csv rows into the population table", pop_total_rows
                )
                print(f"{pop_total_rows} rows inserted into population table")
            ...
            try:
                ...
                connection.commit()
                cursor.execute("SELECT COUNT(*) AS row_count FROM registration;")
                reg_total_rows = cursor.fetchall()[0][0]
                logger.info(
                    "Inserted %s csv rows into the registration table", reg_total_rows
                )
                print(f"{reg_total_rows} rows inserted into registration table")
            except sqlite3.Error as e:
                connection.rollback()
