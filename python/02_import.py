...
#
    ...
    with open(
        "../data/Electric_Vehicle_Title_and_Registration_Activity_20260523.csv", "r"
    ) as file:
        reader = csv.DictReader(file)
    ...
    try:
        ...
        connection.commit()
        print(f"Inserted {len(population_csv_rows)} population rows")
    except sqlite3.Error as e:
        connection.rollback()
        print(f"Population insert failed: {e}")
    ...
    try:
        ...
        connection.commit()
        print(f"Inserted {len(registration_csv_rows)} registration rows")
    except sqlite3.Error as e:
        connection.rollback()
        print(f"Registration insert failed: {e}")

    # Validation Check
    sql_check_population = "SELECT * FROM population LIMIT 10;"
    sql_check_registration = "SELECT * FROM registration LIMIT 10;"
